...
import { LayoutGrid, List, SlidersHorizontal, Activity, Users } from 'lucide-react'
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <nav className="hidden sm:flex items-center gap-1 p-1 rounded-xl bg-white/5 border border-white/10 mr-2">
                <NavTab active={false} onClick={() => router.push('/contacts')} icon={<Users size={13} />} label="Contatos" />
                <NavTab active={true} onClick={() => {}} icon={<Activity size={13} />} label="Ciclos" />
              </nav>
...
function NavTab({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: ReactNode; label: string }) {
  return (
    <motion.button
      onClick={onClick}
      whileTap={{ scale: 0.95 }}
